...
(function ($) {
  ...
  function SelectEditor(args) {
    ...
    this.init = function () {
      ...
      $select.bind("keydown.nav", function (e) {
        // Fix for up/down which bubble up the change to the next row edit
  ...
  function SpinnerEditor(args) {
    var $input,
    	defaultValue,
    	scope = this,
		max = args.column.max || 50;

    this.init = function () {
		$input = $("<INPUT type='text' class='editor-text' maxlength='"+max.toString().length+"' />")
          .appendTo(args.container)
          .focus()
          .spinner({
            min: args.column.min || 0,
            max: max
          });
    };
  ...
  function LongTextEditor(args) {
    ...
    this.init = function () {
      ...
      $("<DIV style='text-align:right'><BUTTON class='btn-green'>Save</BUTTON><BUTTON class='btn-grey'>Cancel</BUTTON></DIV>")
          .appendTo($wrapper);
